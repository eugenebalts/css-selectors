...
export default class GameField {
    ...
    private mainTitle: HTMLHeadElement | null =
        document.querySelector('.main__title');
    private inputHint: HTMLDivElement | null =
        document.querySelector('.input__hint_html');
    ...
    public initialField(): void {
        ...
        if (this.gameField) {
            ...
            for (const car in objectsArray) {
                const carContainer: HTMLDivElement =
                    document.createElement('div');
                carContainer.classList.add(
                    'vehicle__container',
                    `${objectsArray[car].row}`,
                    `${objectsArray[car].image}`,
                );

                if (objectsArray[car].tag)
                    carContainer.setAttribute(
                        'data-tag',
                        `${objectsArray[car].tag}`,
                    );

                const carImage: HTMLImageElement =
                ...
                if (objectsArray[car].classAdditional)
                    carImage.classList.add(
                        `${objectsArray[car].classAdditional}`,
                    );
            ...
            HTMLCode.forEach((string, index) => {
                const carContainer: HTMLDivElement | null =
                    document.querySelector(
                        `.${levelProps.objects[index].image}`,
                    );
    ...
    private setHoverListeners(): void {
        if (this.gameField) {
            window.addEventListener('mouseover', (event) => {
                const HTMLStrings: Array<HTMLElement> | [] = Array.from(
                    document.querySelectorAll('.html-string'),
                );
                const carsArray: Array<HTMLDivElement> | [] = Array.from(
                    document.querySelectorAll('.vehicle__container'),
                );
                if (event.target instanceof HTMLElement) {
                    if (event.target.closest('.vehicle__container')) {
                        const carContainer: HTMLElement | null =
                            event.target.closest('.vehicle__container');
                        if (carContainer) {
                            const carDataAttribute: string | null =
                                carContainer.getAttribute('data-connect');
                            HTMLStrings.forEach((string) => {
                                if (
                                    string.getAttribute('data-connect') ===
                                    carDataAttribute
                                ) {
                    ...
                    if (event.target.classList.contains('html-string')) {
                        const stringDataAttribute: string | null =
                            event.target.getAttribute('data-connect');
                        carsArray.forEach((car) => {
                            const carDataAttribute =
                                car.getAttribute('data-connect');
                            if (stringDataAttribute === carDataAttribute) {
                                car.classList.add('vehicle__container_hovered');
                            } else {
                                car.classList.remove(
                                    'vehicle__container_hovered',
                                );
                            }
                        });
                    } else {
