...
export default class LevelControl {
    ...
    private resetBtn: HTMLButtonElement | null = document.querySelector(
        '.sidebar__reset-btn',
    );
    private burgerMenuBtn: HTMLDivElement | null =
    ...
    public writeLevels(): void {
        ...
        for (const level of LEVELS) {
            const levelTitle: HTMLParagraphElement =
                document.createElement('p');
            levelTitle.classList.add(
                'levels__title',
                `levels__title_${level.level}`,
            );
    ...
    public updateLevels(type = 'update'): void {
        ...
        const isHintUsed: boolean = state.isHintUsed;
        const levelsTitle: NodeListOf<HTMLParagraphElement> =
            document.querySelectorAll('.levels__title');
        levelsTitle.forEach((item, index) => {
            ...
            if (item.classList.contains(`levels__title_${currentLevel}`))
                this.isPassedLevel(
                    currentLevel - 1,
                    levelsTitle[index - 1] as HTMLElement,
                    isHintUsed,
                );
    ...
    private setListener(): void {
        ...
        if (this.resetBtn) {
            this.resetBtn.addEventListener(
                'click',
                this.resetProgress.bind(this),
            );
        }
        if (this.burgerMenuBtn) {
            this.burgerMenuBtn.addEventListener(
                'click',
                this.burgerMenu.bind(this),
            );
        }
    }
    ...
    private switchLevel(): void {
        ...
        navItems.forEach((item) => {
            item.addEventListener('click', () => {
                ...
                const levelAttribute: string | null =
