...
describe('LevelControl', () => {
    ...
    test('Should set class "levels__title_passed" to navigation element if this level was passed', () => {
        ...
        expect(fakeElement1.classList.contains('levels__title_passed')).toBe(
            true,
        );
        expect(fakeElement1.classList.contains('levels__title_hint-used')).toBe(
            true,
        );
        expect(fakeElement2.classList.contains('levels__title_passed')).toBe(
            false,
        );
        expect(fakeElement2.classList.contains('levels__title_hint-used')).toBe(
            false,
        );
        expect(fakeElement3.classList.contains('levels__title_passed')).toBe(
            true,
        );
        expect(fakeElement3.classList.contains('levels__title_hint-used')).toBe(
            false,
        );
    });
});
